...
function conv(src, opt) {
    ...
    try {
        ...
        const blockTokens = BlockLexer.lex(src, options);

        const mdParseResult = Parser.parse(blockTokens, options);

        const topLevelContext = contextify(mdParseResult);

        const processedTopLevelContext = process(topLevelContext);
