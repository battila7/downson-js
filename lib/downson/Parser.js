...
function parseListElement(listElement) {
    ...
    while (elements.length > 0) {
        ...
        if (element.type == 'object terminator') {
            ...
            const value = element.elements.map(parseListElement);
...
function parseElements(originalElements) {
    ...
    while (elements.length > 0) {
        ...
        if (element.type == 'object terminator') {
            ...
            const value = parseTable(element);
...
function parseChildContexts(contexts) {
    const result = {};
    ...
    for (const context of contexts) {
        ...
        if (contextKey) {
            result[contextKey] = parseContext(context);
        }
    }
...
function parseContext(context) {
    const elementData = parseElements(context.elements);
    const childContextData = parseChildContexts(context.childContexts);

    return Object.assign({}, elementData, childContextData);
}

module.exports = function parse(topLevelContext) {
    return parseContext(topLevelContext);
};
