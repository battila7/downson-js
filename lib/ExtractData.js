...
function extractFromListElement(listElement) {
    ...
    while (elements.length > 0) {
        ...
        const currentObject = nestingStack.length > 0 ? nestingStack[nestingStack.length - 1].contents : null;
        ...
        if (element.type == 'object terminator') {
            if (currentKey || currentLiteral) {
                ...
                nestingStack[element.length - 1].contents[obj.key] = obj.contents;
            }
            ...
            const hasNext = elements.length > 1;

            if (hasNext) {
                const nextElement = elements[elements.length - 2];

                if (nextElement.type == 'key metadata') {
                    const key = nextElement.metadata.includes('alias') ? nextElement.alias : element.keyName;
                    ...
                    if (currentKey) {
                        ...
                        if (!nextElement.metadata.includes('left') || nextElement.metadata.includes('right')) {
                            // err
                        } else if (nextElement.metadata.includes('object')) {
                            // err
                        } else {
                            if (currentObject) {
                                currentObject[key] = currentLiteral;
                            } else {
                                // err
                            }
                            currentLiteral = null;
                        ...
                        if (nextElement.metadata.includes('left')) {
                            if (nextElement.metadata.includes('object')) {
                                const obj = nestingStack.pop();

                                nestingStack[element.length - 1].contents[key] = obj.contents;
                            } else {
                                // err
                            }
                        } else if (nextElement.metadata.includes('right')) {
                            if (nextElement.metadata.includes('object')) {
                                nestingStack.push({
                                    isTopLevel: false,
                                    key, // !!!
            ...
            elements.pop();
        } else if (element.type == 'noise' || element.type == 'text') {
            // no need for these
...
function extractFromTable(tableElement) {
    ...
    for (const headerElements in tableElement.header) {
        if (headerElements.length == 1 && headerElements[0].type == 'text') {
            keys.push(headerElements[0].text);
    ...
    for (const row in tableElement.rows) {
        const obj = {};
...
function extractFromElements(elements) {
    ...
    while (elements.length > 0) {
        const element = elements[elements.length - 1];
        const currentObject = nestingStack[nestingStack.length - 1].contents;
...
function extractFromContext(context) {
    const elementData = extractFromContext(context.elements);
