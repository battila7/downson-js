...
const ContextBuilder = {
    ...
    isInvalidHeading(heading) {
        if (heading.inner.length == 1) {
            ...
            const link = heading.inner[1];
            if (link.data.href == 'ignore') {
                // spec: If the ignore marker is followed by some valid link text...
                if (link.data.cap[1].trim() != '' || link.data.title) {
                    this.failures.push(ambiguousSyntax('Invalid ignore alias.', heading));

                    return true
                }
            } else if (link.data.href == 'alias') {
                if (link.data.cap[1].trim() != '' || !link.data.title) {
                    this.failures.push(ambiguousSyntax('Invalid key alias.', heading));
